...
@app.put("/mongodb/items/{item_id}", response_model=dict)
...
@app.delete("/mongodb/items/{item_id}", response_model=dict)
def delete_mongodb_item(item_id: str):
    result = items_collection.delete_one({"_id": ObjectId(item_id)})  # Convert string ID to ObjectId
